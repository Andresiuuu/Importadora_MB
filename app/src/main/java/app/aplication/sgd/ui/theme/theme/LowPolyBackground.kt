package app.aplication.sgd.ui.theme.background
...
fun LowPolyBackground(
    ...
    Canvas(modifier = modifier.fillMaxSize()) {
        ...
        if (polygons.value.isEmpty()) {
            polygons.value = generateLowPolyTriangles(size.width, size.height)
        }
...
fun generateLowPolyTriangles(width: Float, height: Float): List<Polygon> {
    val polygons = mutableListOf<Polygon>()
    val gridRows = 5  // Reducido de 12 a 7
    val gridCols = 4  // Reducido de 8 a 5
...
fun interpolateColor(progress: Float): Color {
    val colors = listOf(
        Color(0xFFCCCCCC), // Blanco
        Color(0xFFA1A0A0), // Gris muy claro
        Color(0xFFB0B0B0), // Gris claro
        Color(0xFF6B6969), // Gris medio
        Color(0xFF4F4F4F), // Gris
        Color(0xFF494848), // Gris oscuro
        Color(0xFF2C2B2B), // Gris muy oscuro
        Color(0xFF101010)  // Negro
    )
...
fun adjustBrightness(color: Color, adjustment: Float): Color {
    return Color(
...
fun DrawScope.drawPolygon(polygon: Polygon) {
    val path = Path().apply {
